...
const normalizeApiError = (value, fallback = 'An unexpected error occurred.') => {
  if (!value) {
    return fallback;
  }
    ...
    if (response) {
      ...
      switch (status) {
        ...
        case 500:
          ...
          break;
        default:
          toast.error(normalizeApiError(data?.error));
      }
...
export const adminService = {
  ...
  async getCameraSources(params = {}) {
    return api.get('/admin/camera-sources', { params });
  },

  async getInferenceEvents(params = {}) {
    return api.get('/admin/inference-events', { params });
  },

  async submitInferenceEvent(payload) {
    return api.post('/admin/inference-events', payload);
  },

  async getAiProcessingJobs(params = {}) {
    return api.get('/admin/ai-processing-jobs', { params });
  },

  async createAiProcessingJob(payload) {
    return api.post('/admin/ai-processing-jobs', payload);
  },

  async updateAiProcessingJobStatus(jobId, payload) {
    return api.put(`/admin/ai-processing-jobs/${jobId}/status`, payload);
  },

  async rerunAiProcessingJob(jobId) {
    return api.post(`/admin/ai-processing-jobs/${jobId}/rerun`);
  },

  async runDemoAiInference(payload = {}) {
    return api.post('/admin/ai-processing-jobs/demo-infer', payload);
  },

  async getDataSources(params = {}) {
    return api.get('/admin/data-sources', { params });
  },

  async getOpenDataImportJobs(params = {}) {
    return api.get('/admin/open-data-import-jobs', { params });
  },

  async getOpenDataOccupancySnapshots(params = {}) {
    return api.get('/admin/open-data-occupancy-snapshots', { params });
  },

  async getParkingLotCandidates(params = {}) {
    return api.get('/admin/parking-lot-candidates', { params });
  },

  async updateParkingLotCandidateReview(candidateId, payload) {
    return api.put(`/admin/parking-lot-candidates/${candidateId}/review`, payload);
  },

  async getParkingOperations(params = {}) {
    return api.get('/admin/parking-operations', { params });
  },
...
export const chatbotService = {
  ...
  async getSuggestions(intent) {
    return api.get(`/chatbot/suggestions/${encodeURIComponent(intent || 'general')}`);
  }
};
