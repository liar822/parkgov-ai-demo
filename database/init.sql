...
CREATE TABLE IF NOT EXISTS camera_sources (
    ...
    name VARCHAR(255) NOT NULL,
    source_kind VARCHAR(50) NOT NULL CHECK (source_kind IN ('image', 'video_file', 'rtsp', 'http_stream', 'manual_demo')),
    source_url TEXT,
    coverage_description TEXT,
    status VARCHAR(50) DEFAULT 'planned' CHECK (status IN ('planned', 'online', 'offline', 'sample', 'disabled')),
...
-- Create ai_processing_jobs table for image/video inference task tracking
...
-- Create parking_slot_rois table for versioned camera/image ROI annotations
...
-- Create arrival_intents table for demo arrival assurance plans.
-- These records do not reserve a real slot and do not trigger payment.
...
-- Drop first so the initialization script can be re-run safely.
...
DROP TRIGGER IF EXISTS update_camera_sources_updated_at ON camera_sources;
CREATE TRIGGER update_camera_sources_updated_at 
    BEFORE UPDATE ON camera_sources 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP TRIGGER IF EXISTS update_parking_slot_rois_updated_at ON parking_slot_rois;
CREATE TRIGGER update_parking_slot_rois_updated_at
    BEFORE UPDATE ON parking_slot_rois
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
DROP TRIGGER IF EXISTS update_arrival_intents_updated_at ON arrival_intents;
CREATE TRIGGER update_arrival_intents_updated_at
    BEFORE UPDATE ON arrival_intents
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Insert sample data
-- Create default admin user (password: admin123)
INSERT INTO users (username, email, password_hash, role) 
VALUES ('admin', 'admin@parking.com', '$2a$10$WRR4hxhFYMlBvDmC2NUntOUW1CtdRrvyJkP.3XZA5AIbXnfN7QxHm', 'admin')
ON CONFLICT (username) DO NOTHING;

-- Create sample user (password: user123)
INSERT INTO users (username, email, password_hash, role) 
VALUES ('user', 'user@parking.com', '$2a$10$ZJkZhZ2pjdUuVG5VIB..s.jQ6yh/h0ihL0dLh0//b5zkZG7.jFGkC', 'user')
ON CONFLICT (username) DO NOTHING;
...
SELECT
    ...
    true
WHERE NOT EXISTS (
    SELECT 1 FROM parking_lots WHERE name = 'Main Parking Lot'
);
...
BEGIN
    -- Get user ID
    SELECT id INTO sample_user_id FROM users WHERE username = 'user' LIMIT 1;

    IF EXISTS (
        SELECT 1
        FROM bookings b
        WHERE b.user_id = sample_user_id
        LIMIT 1
    ) THEN
        RETURN;
    END IF;
...
BEGIN
    ...
    FOR i IN 1..5 LOOP
        ...
        IF NOT EXISTS (
            ...
            WHERE va.parking_lot_id = lot_id
              AND va.video_filename = 'sample-video-' || i || '.mp4'
        ) THEN
...
WHERE u.username IN ('admin', 'user')
  AND NOT EXISTS (
      ...
      WHERE logs.user_id = u.id
        ...
        AND logs.entity_type = 'user'
        AND logs.entity_id = u.id
  );
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE cc.user_id = sample_user_id
          AND intent = 'find_parking'
        LIMIT 1
    ) THEN
        RETURN;
    END IF;
    ...
    VALUES 
        (session_uuid, sample_user_id, 'Where can I find parking?', 'I can help you find available parking! Based on current data, there are 18 available spots in the main parking lot.', 'find_parking', 0.95),
        (session_uuid, sample_user_id, 'How much does it cost?', 'Parking rates are $2 per hour for the first 3 hours, then $5 per hour after that. Daily maximum is $25.', 'check_pricing', 0.90),
        (session_uuid, sample_user_id, 'Thank you!', 'You''re welcome! Feel free to ask if you need any more help with parking.', 'general', 0.85);
END $$;
