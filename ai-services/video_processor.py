...
def build_standard_inference_result(results: Dict,
                                    parking_lot_id: Optional[int] = None,
                                    camera_external_id: Optional[str] = None,
                                    parking_lot_source_id: Optional[str] = None,
                                    input_path: Optional[str] = None,
                                    model_name: str = 'yolov8_parking_slot') -> Dict:
    """Build the normalized JSON shape consumed by the backend inference endpoint."""
    detections = []

    for detection in results.get('slot_detections', []):
        detections.append({
            'slot_id': detection.get('slot_id'),
            'slot_number': detection.get('slot_number'),
            'is_occupied': bool(detection.get('is_occupied', False)),
            'confidence': detection.get('confidence'),
            'predicted_vacancy_seconds': detection.get('predicted_vacancy_seconds', detection.get('predicted_duration', 0))
        })

    payload = {
        'model_name': model_name,
        'input_path': input_path or results.get('video_filename'),
        'inference_timestamp': results.get('timestamp') or datetime.now().isoformat(),
        'notes': 'Generated by ai-services/video_processor.py',
        'detections': detections
    }

    if parking_lot_id is not None:
        payload['parking_lot_id'] = parking_lot_id
    if camera_external_id:
        payload['camera_external_id'] = camera_external_id
    if parking_lot_source_id:
        payload['parking_lot_source_id'] = parking_lot_source_id
...
def main():
    ...
    parser.add_argument('--camera_external_id', help='Camera source external ID')
    parser.add_argument('--parking_lot_source_id', help='Parking lot external/source ID')
    parser.add_argument('--model_name', default='yolov8_parking_slot', help='Model name to include in standard inference JSON')
    ...
    try:
        ...
        results['standard_inference_result'] = build_standard_inference_result(
            results,
